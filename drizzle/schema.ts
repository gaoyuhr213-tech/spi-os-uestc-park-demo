...
/**
 * 行业规则待办：当统一入库发现行业只能使用 fallback 时创建。
 * 以 tenantId + 原始行业文本幂等聚合，不覆盖企业原始行业或证据。
 */
export const industryRuleTodos = mysqlTable("industryRuleTodos", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: varchar("tenantId", { length: 32 }).default("uestc").notNull(),
  todoKey: varchar("todoKey", { length: 196 }).notNull().unique(),
  rawIndustry: varchar("rawIndustry", { length: 128 }).notNull(),
  fallbackScore: int("fallbackScore").default(25).notNull(),
  entityCount: int("entityCount").default(0).notNull(),
  sampleEidsJson: text("sampleEidsJson"),
...
export type IndustryRuleTodoRow = typeof industryRuleTodos.$inferSelect;
